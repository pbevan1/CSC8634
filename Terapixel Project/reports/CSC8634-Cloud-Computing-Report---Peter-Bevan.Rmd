...
```{r pie, echo=FALSE, fig.cap='Proportions of task runtimes per event'}
...
```{r gpugrid, echo=FALSE, message=FALSE, fig.cap='GPU metric histograms'}
...
An unsupervised learning technique can be used to group data like this into clusters. K-means clustering was selected since the clusters look to be highly spherical, which k-means are known to be able to differentiate well.
...
```{r k2, echo=FALSE, fig.cap='k=2 clustering of GPU performance data in space of first two PCs'}
...
```{r k4, echo=FALSE, fig.cap='k=4 clustering of GPU performance data in space of first two PCs'}
#kmeans function with K=2 as identified by above plot
...
The summary statistics for the cluster of GPUs identified as better performing in k=2 clustering:

```{r km2 best summary, echo=FALSE, fig.cap='best of k=2'}
summary(bestgpu_km_2)
```

The summary statistics for the cluster of GPUs identified as worse performing:
...
```{r scatter_km2, echo=FALSE, fig.cap='scatterplot matrix with k=2 clustering'}
...
```{r scatter_km4, echo=FALSE, fig.cap='scatterplot matrix with k=4 clustering'}
#Creating scatterplot matrix with k-means clusters used to colour groups
#clustered on the full dataset.
...
The wrangled 'gpu_performance` data presented in this analysis could quite quickly and easily be used for anomaly detection to to weed out poorly performing GPUs (by serial number) that slowed down the process. This would be the next step for future iterations of the report but was left out on this occasion.
